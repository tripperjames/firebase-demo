...
if ($theme->get('search_module') == 'mod_finder') {
...
if (in_array($position, ['navbar', 'header']) && preg_match('/^(horizontal|stacked)/', $layout)) {
    ...
    $style = $theme->get('header.search_style');

    // Deprecated
    if ($style === null) {
        if (strrpos($layout, 'horizontal-', 0) !== false) {
...
<?php elseif (false && in_array($style, ['dropdown', 'justify'])) : ?>
    ...
    $drop = [
        ...
        'boundary' => $navbar['dropdown_align'] ? '!uk-navbar-container' : false,
