...
class Builder
{
    ...
    public function addTypePath($paths, $basePath = null)
    {
        foreach ((array) $paths as $path) {
            ...
            $path = File::resolvePath($basePath, $path);
            $types = array_map($this->loader, glob($path, GLOB_NOSORT) ?: []);
     ...
     * @return self
     */
    public function addTransform($context, callable $transform)
    {
        $this->transforms[$context][] = $transform;
        $this->resolved = [];
        ...
        if (isset($node->type, $this->types[$node->type])) {
            ...
            if (!empty($node->children)) {
                ...
                foreach ($node->children as $child) {
                    if ($this->applyTransforms($child, $childParams)) {
                        $children[] = $child;
                    }
                }
